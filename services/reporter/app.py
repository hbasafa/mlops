...
"""
This is a demo service for Predictor metrics integration with Prometheus and Grafana.
...
from predictor.pipeline.column_mapping import ColumnMapping
from predictor.model_monitoring import (
    ModelMonitoring,
    DataDriftMonitor,
    DataQualityMonitor,
...
class MonitoringService:
    ...
    def __init__(self, datasets: Dict[str, LoadedDataset], window_size: int):
        ...
        for dataset_info in datasets.values():
            ...
            self.monitoring[dataset_info.name] = ModelMonitoring(
                monitors=[
                    PREDICTOR_MONITORS_MAPPING[k]() for k in dataset_info.monitors
                ],
                options=[],
            )
    ...
    def iterate(self, dataset_name: str, new_rows: pd.DataFrame):
        ...
        for metric, value, labels in self.monitoring[dataset_name].metrics():
            metric_key = f"predictor:{metric.name}"
